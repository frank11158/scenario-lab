...
export function BriefPanel({ study, run, revisions, onChange, onNavigate, onAccept, exportUrl }: Props) {
  ...
  return <div className="panel-stack" data-testid="brief-panel">
    ...
    <section className="card gap-summary"><div><div className="mini-label">Unresolved evidence</div><h3>{study.evidenceGaps.length} gap{study.evidenceGaps.length === 1 ? "" : "s"} · {study.research.contradictions.filter((item) => item.status === "open").length} conflict{study.research.contradictions.filter((item) => item.status === "open").length === 1 ? "" : "s"}</h3><ul>{study.evidenceGaps.map((gap) => <li key={gap}>{gap}</li>)}{study.evidence.filter((item) => item.support.status === "unsupported" || item.support.status === "unreviewed").map((item) => <li key={item.id}>{item.support.status}: {item.claim}</li>)}{study.research.unavailableSources.map((item) => <li key={item.id}>Unavailable: {item.locator}</li>)}</ul></div><button className="button secondary" onClick={() => onNavigate("evidence")}>Review evidence</button></section>
    ...
    <section className="card history-card">
      ...
      <div className="timeline">{revisions.map((revision) => <div className="timeline-row" key={revision.id}><span>{revision.ordinal}</span><div><b>{revision.summary}</b><small>{revision.kind} · {new Date(revision.acceptedAt).toLocaleString()}</small></div><div className="export-links"><a href={exportUrl("markdown", revision.id)}>MD</a><a href={exportUrl("json", revision.id)}>JSON</a></div></div>)}{!revisions.length && <p className="empty">No accepted revisions yet.</p>}</div>
    </section>
  </div>;
}
